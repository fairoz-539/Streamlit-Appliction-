...
def main_app():
    ...
    # Select custom color for headings
    heading_color = st.color_picker("Select Heading Color:", value="#393b3d")
            
    # Select custom color for text
    text_color = st.color_picker("Select Text Color:", value="#307bd1")
            
    # Generate Answer
    st.session_state.Gen_Ans = st.button("Generate Answer")

    if st.session_state.Gen_Ans:
    ...
    # Generate PDF
    st.session_state.Gen_PDF = st.button("Generate PDF")
            
    if st.session_state.Gen_PDF and st.session_state.Answer:
        try:
            pdf_file_path = generate_pdf(st.session_state.question, st.session_state.Answer, name, rno, sec, sub, assign_num, font_family=st.session_state.font_family, heading_color=heading_color, text_color=text_color)
            with open(pdf_file_path, "rb") as pdf_file:
                pdf_base64 = base64.b64encode(pdf_file.read()).decode("utf-8")
                pdf_data_uri = f"data:application/pdf;base64,{pdf_base64}"
                st.markdown(f'<a href="{pdf_data_uri}" download="output.pdf">Download PDF</a>', unsafe_allow_html=True)
        except Exception as e:
            st.error("An error occurred while generating the PDF: {}".format(e))

# Function to display the home page
def home_page():
    st.title("Welcome to the Assignment Writer App!")
    ...
    if submit_button:
        user = validate_user(st.session_state.username, st.session_state.password)
        if user:
...
def signup_page():
    ...
    if submit_button:
        if username and password:
            ...
            if profile_pic:
                profile_pic_path = os.path.join("profile_pics", username + "_" + profile_pic.name)
                with open(profile_pic_path, "wb") as f:
                    f.write(profile_pic.getbuffer())

            try:
                add_user(username, password, profile_pic_path)
